...
## Iteration 6 — bulk content seed (51 professional blog posts)
- Generated **51 professional blog posts** via `scripts/gen-50-posts.py` and merged into `src/db/blog.json` (total now 53: 2 original + 51 new).
- Categories: latest technology/news (10), tutorials (11), programming (10), open-source news (10), open-source AI agents (11). Heavy tag overlap: `ai`(22), `agents`(21), `open-source`(21), `nextjs`(11), `tutorial`(11), `programming`(11).
- Each post matches the exact 10-field schema (no coverImage/author — fields don't exist). **Media**: inline GFM image markdown `![alt](https://picsum.photos/seed/<slug>/1200/600)` — stable, license-free, deterministic placeholder cover images (no `/public` images dir exists; renderer supports inline images). No video (static export + free tier; video needs hosting).
- Dates spread 2026-05-28..2026-08-06 (ISO `T00:00:00.000Z`), all `published: true`, `locale: 'en'`, `id === slug` (ASCII kebab). Slugs all unique.
- **Build green** with static export: 53 blog routes prerendered as `● (SSG)`, sitemap has 53 blog URLs, `public/feed.xml` has 53 items (regenerated by `prebuild` → `scripts/gen-feed.mjs`). Per-post metadata (title/OG description) generated from seed at build.
- **Content renders at runtime from Firestore**, not from the static HTML — public blog pages are Client Components fetching Firestore on mount (Iteration 5 design). The static HTML contains the shell + build-time metadata (title, OG tags, canonical). To make the 51 posts actually display, run `npm run seed:firestore` (Admin SDK, uses `doc(post.id)` to preserve ids) AFTER the first Firebase deploy. The seed is the source of truth for both static params/metadata/sitemap/feed AND the Firestore runtime data.
- `scripts/gen-50-posts.py` is idempotent: skips slugs already present, so it can be re-run safely. Keep it for future bulk additions.
- Gotcha when editing the generator: f-strings inside JS/TS/YAML code blocks need `{{`/`}}` escaping for literal braces; `${...}` template-literal interpolation and `import { x }` (unescaped braces) both break the f-string. Use placeholder prose or double the braces.
